...
ssh root@$TARGETIP killall loader

# remove old executable on target
ssh root@$TARGETIP rm $OLDPROGRAMPATH_REMOTE
//ssh root@$TARGETIP mkdir $OLDPROGRAMPATH_REMOTE
scp -r $PATH_LOCAL root@$TARGETIP:$PATH_REMOTE

# copy over new executable
scp $EXECUTABLE_LOCAL root@$TARGETIP:$EXECUTABLE_REMOTE

# start gdb on target (IS ONE LONG COMMAND)#
ssh -n -f root@$TARGETIP "sh -c 'cd /root; nohup gdbserver localhost:3000 /root/fbShot > /dev/null 2>&1 &'"
